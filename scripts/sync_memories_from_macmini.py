#!/usr/bin/env python3
"""
data_macmini의 memories.db에서 data/memories.db에 없는 메모리를 동기화하는 스크립트

Usage:
    python scripts/sync_memories_from_macmini.py [--dry-run]
"""
...
# 데이터베이스 경로
SOURCE_DB = ROOT_DIR / "data_macmini" / "memories.db"
TARGET_DB = ROOT_DIR / "data" / "memories.db"
...
def main():
    parser = argparse.ArgumentParser(
        description="data_macmini의 메모리를 data로 동기화"
    )
    parser.add_argument(
        '--dry-run',
        action='store_true',
    ...
    if args.dry_run:
        ...
        print(f"   python scripts/sync_memories_from_macmini.py")

        ...
        if response.status_code == 200:
            ...
            if "result" in result:
                ...
                if isinstance(batch_result, dict) and "results" in batch_result:
                    ...
                    for i, op_result in enumerate(batch_result["results"], 1):
                        ...
                        if op_result.get("success"):
                            if op_result["type"] == "add":
                                print(
                                    f"  Memory ID: {op_result.get('memory_id', 'N/A')}"
                                )
                                ...
                                for mem in results[:3]:
                                    print(
                                        f"    - {mem.get('id', 'N/A')[:8]}... : {mem.get('content', '')[:50]}..."
                                    )
                                    print(
                                        f"      Similarity: {mem.get('similarity_score', 0):.4f}"
                                    )
                        else:
                    ...
                    print(
                        f"  Successful: {sum(1 for r in batch_result['results'] if r.get('success'))}"
                    )
                    print(
                        f"  Failed: {sum(1 for r in batch_result['results'] if not r.get('success'))}"
                    )

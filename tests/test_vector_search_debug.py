...
async def debug_vector_search():
    ...
    try:
                ...
                for i, row in enumerate(raw_results, 1):
                    ...
                    if i == 1:
                        ...
                        if memory_embedding_row:
                            ...
                            memory_embedding_array = np.frombuffer(memory_embedding_bytes, dtype=np.float32)
                            
                            # 코사인 유사도 직접 계산
                            query_array = np.array(query_embedding_list, dtype=np.float32)
                            
                            # 정규화
                            query_norm = query_array / np.linalg.norm(query_array)
                            memory_norm = memory_embedding_array / np.linalg.norm(memory_embedding_array)
        ...
        if understood_memories:
            threshold_embedding = np.array(embedding_service.embed("threshold"), dtype=np.float32)
            threshold1_embedding = np.array(embedding_service.embed("threshold1"), dtype=np.float32)
            ...
            for i, memory in enumerate(understood_memories, 1):
                ...
                threshold1_norm = threshold1_embedding / np.linalg.norm(threshold1_embedding)
        ...
        if vector_table_exists:
            ...
            # 벡터 테이블 레코드 수
            vector_count = await db.fetchone("SELECT COUNT(*) as count FROM memory_embeddings")

...
def valid_tags(draw):
    ...
    return draw(
        st.lists(
            st.text(min_size=1, max_size=30).filter(lambda x: x.strip()), max_size=10
        )
    )
...
class TestProjectProperties:
    ...
    @pytest.mark.asyncio
    @pytest.mark.property
    async def test_project_update_persistence(
        self, temp_db, project_id, name, description, tech_stack
    ):
        """
...
class TestPinStatusTransitions:
    ...
    @given(initial_status=valid_pin_status(), target_status=valid_pin_status())
...
class TestUserFiltering:
    ...
    def test_user_id_default_value(self, user_id):
        ...
        **Validates: Requirements 8.2**
        ...
        # 기본값 로직
        effective_user_id = (
            user_id.strip() if user_id and user_id.strip() else "default"
        )

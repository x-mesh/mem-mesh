...
async def test_pins_table_has_optimization_columns(test_db):
    ...
    assert 'promoted_to_memory_id' in column_names, "pins.promoted_to_memory_id 컬럼이 없습니다"
...
async def test_sessions_table_has_optimization_columns(test_db):
    ...
    # 새로운 컬럼 확인
    assert 'initial_context_tokens' in column_names, "sessions.initial_context_tokens 컬럼이 없습니다"
    assert 'total_loaded_tokens' in column_names, "sessions.total_loaded_tokens 컬럼이 없습니다"
    assert 'total_saved_tokens' in column_names, "sessions.total_saved_tokens 컬럼이 없습니다"
...
async def test_token_usage_table_exists(test_db):
    ...
    expected_columns = [
        ...
        'query', 'tokens_used', 'tokens_saved', 'optimization_applied', 'created_at'
    ]
...
async def test_optimization_indexes_exist(test_db):
    ...
    # session_stats 인덱스
    assert 'idx_session_stats_session' in index_names, "idx_session_stats_session 인덱스가 없습니다"
    assert 'idx_session_stats_timestamp' in index_names, "idx_session_stats_timestamp 인덱스가 없습니다"
    assert 'idx_session_stats_event_type' in index_names, "idx_session_stats_event_type 인덱스가 없습니다"
    
    # token_usage 인덱스
    assert 'idx_token_usage_project' in index_names, "idx_token_usage_project 인덱스가 없습니다"
    assert 'idx_token_usage_session' in index_names, "idx_token_usage_session 인덱스가 없습니다"
    assert 'idx_token_usage_created' in index_names, "idx_token_usage_created 인덱스가 없습니다"
    assert 'idx_token_usage_operation' in index_names, "idx_token_usage_operation 인덱스가 없습니다"
    
    # pins 테이블 추가 인덱스
    assert 'idx_pins_promoted' in index_names, "idx_pins_promoted 인덱스가 없습니다"
    assert 'idx_pins_auto_importance' in index_names, "idx_pins_auto_importance 인덱스가 없습니다"
...
async def test_foreign_key_constraints(test_db):
    ...
    assert any(fk['table'] == 'sessions' for fk in fks), "session_stats.session_id 외래 키가 sessions를 참조하지 않습니다"
    ...
    assert any(fk['table'] == 'sessions' for fk in fks), "token_usage.session_id 외래 키가 sessions를 참조하지 않습니다"
...
async def test_insert_session_stats_record(test_db):
    ...
    # 조회 확인
    cursor = await test_db.execute(
        "SELECT * FROM session_stats WHERE id = ?",
        (stat_id,)
    )
    result = cursor.fetchone()

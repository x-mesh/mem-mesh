...
from app.core.services.context_optimizer import ContextLoadingParams, ContextOptimizer
...
class TestAdjustForIntent:
    ...
    async def test_debug_intent(self, context_optimizer):
        ...
        intent = MockSearchIntent(
            intent_type="debug", urgency=0.5, specificity=0.5, temporal_focus="any"
        )

        expand, limit, min_importance = await context_optimizer.adjust_for_intent(
            intent=intent, project_id="test-project", base_limit=10
        )
    ...
    async def test_explore_intent(self, context_optimizer):
        ...
        """
        intent = MockSearchIntent(
            intent_type="explore", urgency=0.3, specificity=0.3, temporal_focus="any"
        )
    ...
    async def test_implement_intent(self, context_optimizer):
        ...
        """
        intent = MockSearchIntent(
            intent_type="implement", urgency=0.5, specificity=0.6, temporal_focus="any"
        )
    ...
    async def test_recent_temporal_focus(self, context_optimizer):
        """최근 시간 초점은 더 많은 결과"""
        intent = MockSearchIntent(
            intent_type="review", urgency=0.5, specificity=0.5, temporal_focus="recent"
        )
    ...
    async def test_base_limit_consideration(self, context_optimizer):
        ...
        intent = MockSearchIntent(
            intent_type="explore", urgency=0.5, specificity=0.5, temporal_focus="any"
        )
...
class TestLoadContextForSearch:
    ...
    async def test_load_context_with_debug_intent(
        self, context_optimizer, mock_session_service, sample_session_context
    ):
        """Debug 의도로 맥락 로드
        ...
        context = await context_optimizer.load_context_for_search(
            query="test query", project_id="test-project", intent=intent
        )
    ...
    async def test_load_context_no_session(
        self, context_optimizer, mock_session_service
    ):
        """세션이 없는 경우 None 반환"""
        ...
        context = await context_optimizer.load_context_for_search(
            query="test query", project_id="nonexistent-project", intent=intent
        )
...
class TestTokenEstimation:
    ...
    def test_estimate_summary_mode(self, context_optimizer, sample_session_context):
        ...
        estimated = context_optimizer._estimate_context_tokens(
            context=sample_session_context, expand=False
        )
    ...
    def test_estimate_expand_mode(self, context_optimizer, sample_session_context):
        ...
        """
        estimated = context_optimizer._estimate_context_tokens(
            context=sample_session_context, expand=True
        )

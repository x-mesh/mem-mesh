...
async def test_search_relevance():
    ...
    test_queries = [
        ...
        {"query": "검색 품질", "expected_keywords": ["검색", "품질", "search", "quality"]},
        ...
        {"query": "UnifiedSearchService", "expected_keywords": ["unified", "search", "service"]},
        {"query": "노이즈 필터", "expected_keywords": ["노이즈", "필터", "noise", "filter"]},
        {"query": "MCP", "expected_keywords": ["mcp", "protocol"]},
        {"query": "데이터베이스", "expected_keywords": ["데이터베이스", "database", "db", "sqlite"]},
        {"query": "벡터 검색", "expected_keywords": ["벡터", "검색", "vector", "search"]},
    ]
    ...
    for i, test_case in enumerate(test_queries, 1):
        ...
        try:
            ...
            for j, res in enumerate(result.results, 1):
                ...
                matched_keywords = [kw for kw in expected_keywords if kw in content_lower]
                
                print(f"   {status} 결과 {j}: 점수={res.similarity_score:.3f}, "
                      f"관련성={reason}, "
            ...
            print(f"   관련성 비율: {relevant_count}/{len(result.results)} ({relevance_rate:.1f}%)")
    ...
    return {
        ...
        "relevance_rate": total_relevant / total_results * 100 if total_results > 0 else 0
    }
    ...
    for query in detailed_queries:
        ...
        try:
            ...
            for i, res in enumerate(result.results, 1):
                ...
                print(f"{i:2d}. {status} 점수: {res.similarity_score:.3f} | 관련성: {reason}")

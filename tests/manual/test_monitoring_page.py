...
async def generate_test_metrics():
    ...
    try:
        ...
        alert_types = [
            ('low_similarity', 'warning', '평균 유사도가 임계값 미만입니다: 45% < 50%', 0.45, 0.50),
            ('high_no_results', 'warning', '결과없음 비율이 임계값 초과입니다: 25% > 20%', 25.0, 20.0),
            ('slow_response', 'error', '평균 응답 시간이 임계값 초과입니다: 1200ms > 1000ms', 1200, 1000),
        ]
        
        for i, (alert_type, severity, message, metric_val, threshold_val) in enumerate(alert_types):
...
async def clear_test_metrics():
    ...
    try:
        ...
        await db.execute("DELETE FROM embedding_metrics WHERE model_name LIKE '%test%' OR timestamp > datetime('now', '-1 day')")
        await db.execute("DELETE FROM alerts WHERE timestamp > datetime('now', '-1 day')")

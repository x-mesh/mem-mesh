...
async def test_work_tracking_integration():
    ...
    try:
        ...
        session = await session_service.get_or_create_active_session(
            "integration-test-project", "test-user"
        )
        ...
        print("\n6. 세션 컨텍스트 로드 테스트:")
        session_context = await session_service.resume_last_session(
            "integration-test-project", "test-user", expand=True
        )
        ...
        print(
            f"   중요도 {completed_pin.importance}인 완료된 Pin에 대한 승격 제안: {should_promote}"
        )
        ...
        ended_session = await session_service.end_session(
            session.id, "Completed initial development tasks"
        )

        ...
        for mode in modes:
            ...
            try:
                result = await backend.unified_search_service.search(
                    query=test_query, limit=5, search_mode=mode
                )
...
async def test_recency_weight():
    ...
    for weight in weights:
        ...
        search_params = SearchParams(query=test_query, limit=5, recency_weight=weight)
...
async def test_category_filtering():
    ...
    for category in categories:
        ...
        search_params = SearchParams(query=test_query, category=category, limit=5)
        ...
        for i, r in enumerate(result.results[:3], 1):
            print(
                f"  {i}. [{r.similarity_score:.3f}] [{r.category}] {r.content[:50]}..."
            )

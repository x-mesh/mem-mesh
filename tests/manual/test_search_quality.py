...
class SearchQualityTester:
    ...
    async def test_search_quality(self):
        ...
        results = {"basic": [], "enhanced": []}

        for test_case in self.test_cases:
            print(f"\n🔍 Testing: '{test_case['query']}'")
            print(
                f"   Expected: {test_case['expected_category']} ({test_case['intent']})"
            )
    ...
    async def _test_basic_search(self, test_case: Dict) -> Dict:
        ...
        response = await self.basic_search.search(
            query=test_case["query"], limit=5, search_mode="hybrid"
        )

        search_time = time.time() - start_time

        # Evaluate results
        evaluation = self._evaluate_results(test_case, response.results, search_time)
    ...
    def _evaluate_results(
        self, test_case: Dict, results: List, search_time: float
    ) -> Dict:
    ...
    def _compare_results(self, test_case: Dict, basic: Dict, enhanced: Dict):
        ...
        print(
            f"   Time: Basic {basic['time']:.3f}s vs Enhanced {enhanced['time']:.3f}s "
        ...
        print(
            f"   Category Match: Basic {'✓' if basic_cat else '✗'} vs "
        ...
        print(
            f"   Keyword Coverage: Basic {basic_kw:.1%} vs Enhanced {enhanced_kw:.1%}"
        )
        ...
        if "intent_match" in enhanced["evaluation"]:
            ...
            print(
                f"   Intent Detection: {'✓' if intent_match else '✗'} "
        ...
        if "avg_quality_score" in enhanced["evaluation"]:
            print(
                f"   Avg Quality Score: {enhanced['evaluation']['avg_quality_score']:.3f}"
            )
    ...
    def _generate_summary_report(self, results: Dict):
        ...
        for service_name, service_results in results.items():
            ...
            category_matches = sum(
                1 for r in service_results if r["evaluation"]["category_match"]
            )
            print(
                f"   Category Accuracy: {category_matches}/{len(service_results)} "
                f"({category_matches/len(service_results)*100:.0f}%)"
            )

            # Keyword coverage
            keyword_coverages = [
                r["evaluation"]["keyword_coverage"] for r in service_results
            ]
            print(f"   Avg Keyword Coverage: {statistics.mean(keyword_coverages):.1%}")

            # Relevance score
            relevance_scores = [
                r["evaluation"]["relevance_score"] for r in service_results
            ]
            ...
            if service_name == "enhanced":
                ...
                print(
                    f"   Intent Accuracy: {intent_matches}/{len(service_results)} "
                    f"({intent_matches/len(service_results)*100:.0f}%)"
                )

                # Quality scores
                quality_scores = [
                    r["evaluation"].get("avg_quality_score", 0) for r in service_results
                ]
                if any(quality_scores):
                    print(
                        f"   Avg Quality Score: {statistics.mean(quality_scores):.3f}"
                    )
        ...
        improvement = (
            ((enhanced_avg - basic_avg) / basic_avg) * 100 if basic_avg > 0 else 0
        )
    ...
    async def benchmark_performance(self):
        ...
        for query in queries:
            ...
            await self.enhanced_search.search(
                query, limit=5, search_mode="smart", performance_mode="fast"
            )
        ...
        print(
            f"   P95 latency: {sorted(enhanced_times)[int(len(enhanced_times)*0.95)]:.3f}s"
        )

        # Compare
        overhead = (
            (statistics.mean(enhanced_times) - statistics.mean(basic_times))
            / statistics.mean(basic_times)
        ) * 100
    ...
    async def test_edge_cases(self):
        ...
        for query in edge_cases:
            ...
            try:
                # Test enhanced search
                response = await self.enhanced_search.search(
                    query=query, limit=3, search_mode="smart"
                )

...
async def test_response_compression():
    ...
    storage = DirectStorageBackend(
        memory_service=memory_service, search_service=search_service, db=db
    )
    ...
    # 1. 표준 응답 (압축 없음)
    print("\n1️⃣ 표준 응답 (압축 없음)")
    result_standard = await handlers_uncompressed.search(query=test_query, limit=3)
    ...
    result_minimal = await handlers_compressed.search(
        query=test_query, limit=3, response_format="minimal"
    )
    ...
    print(
        f"   절감: {standard_size - minimal_size} bytes ({(1 - minimal_size/standard_size)*100:.1f}%)"
    )
    print(
        f"   샘플: {json.dumps(result_minimal, ensure_ascii=False, indent=2)[:200]}..."
    )

    # 3. Compact 압축
    print("\n3️⃣ Compact 압축 (요약)")
    result_compact = await handlers_compressed.search(
        query=test_query, limit=3, response_format="compact"
    )
    ...
    print(
        f"   절감: {standard_size - compact_size} bytes ({(1 - compact_size/standard_size)*100:.1f}%)"
    )
    print(
        f"   샘플: {json.dumps(result_compact, ensure_ascii=False, indent=2)[:300]}..."
    )
    ...
    if result_standard.get("results"):
        ...
        context_compact = await handlers_compressed.context(
            memory_id=first_id, depth=1, response_format="compact"
        )
        ...
        print(
            f"   절감: {context_standard_size - context_compact_size} bytes ({(1 - context_compact_size/context_standard_size)*100:.1f}%)"
        )
        print(
            f"   샘플: {json.dumps(context_compact, ensure_ascii=False, indent=2)[:300]}..."
        )
    ...
    print(
        f"   Minimal 검색: ~{minimal_size // 4} tokens (절감: ~{(standard_size - minimal_size) // 4} tokens)"
    )
    print(
        f"   Compact 검색: ~{compact_size // 4} tokens (절감: ~{(standard_size - compact_size) // 4} tokens)"
    )
    ...
    print(
        f"✅ Context 압축: {(1 - context_compact_size/context_standard_size)*100:.1f}% 절감"
        if result_standard.get("results")
        else ""
    )
    print("\n💡 권장 사용법:")

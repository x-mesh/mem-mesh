...
class PerformanceTest:
    ...
    async def test_search_with_cache(self) -> Dict[str, Any]:
        ...
        for round_num in range(2):
            ...
            for query in self.test_queries:
                ...
                print(
                    f"    Query '{query[:30]}...': {query_time:.3f}s {'[CACHE HIT]' if query_time < 0.01 else ''}"
                )
    ...
    async def test_batch_add_vs_individual(self) -> Dict[str, Any]:
        ...
        for content in self.test_contents[:5]:  # Test with 5 items
            ...
            result = await self.memory_service.create(
                content=content, category="test", source="performance_test"
            )
            add_time = time.time() - add_start
            individual_results.append(
                {"content": content[:50], "time": add_time, "status": result.status}
            )
    ...
    async def test_batch_search(self) -> Dict[str, Any]:
        ...
        batch_result = await self.batch_handler.batch_search(
            queries=self.test_queries[5:10], limit=3
        )
        batch_total = time.time() - batch_start
    ...
    async def test_cache_statistics(self) -> Dict[str, Any]:
        ...
        print(
            f"  Embedding cache hit rate: {stats['caches']['embedding']['hit_rate']:.2%}"
        )
    ...
    async def run_all_tests(self) -> None:
        ...
        print("\n📊 Search Performance:")
        print(
            f"  Without cache: {results['search_without_cache']['average_time']:.3f}s avg"
        )
        ...
        print(
            f"  Time saved (search): {results['batch_search']['individual_time'] - results['batch_search']['batch_time']:.2f}s"
        )
        ...
        print(
            f"  Batch operations speedup: {((results['batch_add']['speedup'] + results['batch_search']['speedup']) / 2):.2f}x"
        )

...
@pytest.fixture
async def session_service(db, mock_embedding_service):
    """SessionService 인스턴스 (mock embedding으로 promote 시 모델 로드 방지)"""
    return SessionService(db, embedding_service=mock_embedding_service)
...
class TestResumeWithTokenTracking:
    ...
    async def test_resume_with_no_session(self, session_service):
        ...
        context, token_info = await session_service.resume_with_token_tracking(
            project_id="nonexistent-project", expand=False
        )
    ...
    async def test_resume_with_expand_false(self, session_service, pin_service):
        ...
        await pin_service.create_pin(
            ...
            content="This is a test pin with some content — verifying that session resume correctly loads and displays pins with proper expansion tiers",
            importance=3,
        )

        await pin_service.create_pin(
            project_id="test-project",
            content="Another test pin with more detailed content here — second pin in session to test multi-pin display ordering and importance-based sorting",
            importance=4,
        )
    ...
    async def test_resume_with_expand_true(self, session_service, pin_service):
        ...
        await pin_service.create_pin(
            ...
            content="Pin content for expansion test — testing that smart expansion mode correctly applies four-tier matrix based on status and importance levels",
            importance=3,
        )

        # expand=true로 재개
        context, token_info = await session_service.resume_with_token_tracking(
            project_id="test-project2", expand=True, limit=10
        )
    ...
    async def test_resume_with_limit(self, session_service, pin_service):
        ...
        context, token_info = await session_service.resume_with_token_tracking(
            project_id="test-project3", expand=True, limit=2
        )
...
class TestEndWithAutoPromotion:
    ...
    async def test_auto_promotion_threshold_4(self, session_service, pin_service):
        ...
        # 세션 종료 및 자동 승격
        result = await session_service.end_with_auto_promotion(
            session_id=pin_high.session_id, auto_promote_threshold=4
        )
    ...
    async def test_auto_promotion_custom_threshold(self, session_service, pin_service):
        ...
        # 임계값 3으로 설정
        result = await session_service.end_with_auto_promotion(
            session_id=pin1.session_id, auto_promote_threshold=3
        )
    ...
    async def test_no_promotion_for_incomplete_pins(self, session_service, pin_service):
        ...
        pin_open = await pin_service.create_pin(
            ...
            content="Open pin with high importance — planning phase for microservices migration of the payment processing module with event sourcing",
            importance=5,
        )

        pin_completed = await pin_service.create_pin(
            project_id="incomplete-test",
            content="Completed pin with high importance — finished implementing OAuth2 PKCE flow with token refresh and multi-factor authentication support",
            importance=5,
        )
    ...
    async def test_token_savings_calculation(
        ...
        pin = await pin_service.create_pin(
            ...
            content="Test pin for token savings — measuring token efficiency of compact versus smart versus full expansion modes across different pin counts",
            importance=4,
        )
...
class TestGetSessionStatistics:
    ...
    async def test_statistics_with_no_sessions(self, session_service):
        """세션이 없을 때 통계"""
        stats = await session_service.get_session_statistics(project_id="empty-project")
    ...
    async def test_statistics_with_multiple_sessions(
        self, session_service, pin_service
    ):
        """여러 세션의 통계 계산"""
        # 첫 번째 세션
        pin1 = await pin_service.create_pin(
    ...
    async def test_statistics_with_date_filter(self, session_service, pin_service):
        ...
        stats = await session_service.get_session_statistics(
            project_id="date-filter-project", start_date=yesterday, end_date=today
        )
    ...
    async def test_statistics_all_projects(self, session_service, pin_service):
        ...
        # 여러 프로젝트에 핀 생성
        await pin_service.create_pin(
            project_id="project-a", content="Pin in project A", importance=3
        )
        await pin_service.create_pin(
            project_id="project-b", content="Pin in project B", importance=4
        )
...
class TestIntegration:
    ...
    async def test_full_workflow_with_token_tracking(
        ...
        context1, token_info1 = await session_service.resume_with_token_tracking(
            project_id=project_id, expand=False
        )
        # 첫 재개는 세션이 없을 수 있음
        ...
        pin1 = await pin_service.create_pin(
            ...
            content="Important feature implementation — building the real-time notification system with WebSocket support and message queue integration for reliable delivery",
            importance=5,
        )
        ...
        end_result = await session_service.end_with_auto_promotion(
            session_id=pin1.session_id, auto_promote_threshold=4
        )
        ...
        # 6. 통계 조회
        stats = await session_service.get_session_statistics(project_id=project_id)

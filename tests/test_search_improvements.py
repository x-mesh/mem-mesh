...
async def test_score_normalization():
    ...
    print(
        f"  통계: min={calibration['stats'].min_score:.3f}, "
...
async def test_search_with_normalization():
    ...
    # 테스트 쿼리
    test_queries = ["mem-mesh", "검색 품질", "UnifiedSearchService", "캐싱", "MCP"]
...
async def test_combined_improvements():
    ...
    for test_case in test_cases:
        ...
        if result.results:
            ...
            print(
                f"{query:<20} {description:<15} {len(result.results):<5} "
                f"{avg_score:<10.3f} {search_time*1000:<10.1f}"
            )
        else:
            print(
                f"{query:<20} {description:<15} {'0':<5} {'-':<10} {search_time*1000:<10.1f}"
            )

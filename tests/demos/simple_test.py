                        ...
                        if "id" in structured:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if (
                                        "result" in search_data
                                        and "structuredContent" in search_data["result"]
                                    ):
                                        ...
                                        if "memories" in search_result:
                                            ...
                                            print(
                                                f"✅ 검색 결과: {len(memories)}개 메모리 발견"
                                            )
                                            for memory in memories:
                                                print(
                                                    f"   - ID: {memory.get('id')}, 내용: {memory.get('content', '')[:50]}..."
                                                )
                                        else:
                                            print(
                                                f"⚠️ 검색 결과에 memories가 없음: {search_result}"
                                            )
                                    else:
                                        print(
                                            f"⚠️ 예상과 다른 검색 응답: {search_data}"
                                        )
                                except json.JSONDecodeError:
                                    print(
                                        f"⚠️ 검색 응답 JSON 파싱 실패: {search_response}"
                                    )
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if (
                                        "result" in stats_data
                                        and "structuredContent" in stats_data["result"]
                                    ):
                                        ...
                                        print("✅ 통계 조회 성공!")
                                        print(
                                            f"   - 총 메모리 수: {stats.get('total_memories', 0)}"
                                        )
                                        print(
                                            f"   - 프로젝트 수: {stats.get('total_projects', 0)}"
                                        )
                                    else:
                                        print(f"⚠️ 예상과 다른 통계 응답: {stats_data}")
                                except json.JSONDecodeError:
                                    print(
                                        f"⚠️ 통계 응답 JSON 파싱 실패: {stats_response}"
                                    )

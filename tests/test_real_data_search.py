...
async def extract_search_terms_from_db() -> Dict[str, List[str]]:
    ...
    for row in rows:
        ...
        for word in words:
            ...
            if 3 <= len(clean_word) <= 15 and clean_word not in search_terms["단일_키워드"]:
                search_terms["단일_키워드"].append(clean_word)
        
        # 기술 용어 패턴 (영어 대문자 포함, 특수문자 포함)
        tech_patterns = [
            "API", "MCP", "SQLite", "FastAPI", "Python", "async", "await",
            "embedding", "vector", "search", "cache", "database", "query",
            "token", "optimization", "performance", "quality", "filter"
        ]
        for word in words:
            for pattern in tech_patterns:
                if pattern.lower() in word.lower() and word not in search_terms["기술_용어"]:
        ...
        for word in words:
            if any('\uac00' <= c <= '\ud7a3' for c in word):
                clean_word = word.strip('.,!?()[]{}":;')
                if 2 <= len(clean_word) <= 10 and clean_word not in search_terms["한국어_용어"]:
        ...
        for i in range(len(words) - 1):
            ...
            if 5 <= len(clean_phrase) <= 30 and clean_phrase not in search_terms["복합_쿼리"]:
...
async def test_real_data_search():
    ...
    # 1. 프로젝트명 검색
    for project in random.sample(search_terms["프로젝트명"], min(5, len(search_terms["프로젝트명"]))):
    ...
    # 4. 단일 키워드 검색
    for keyword in random.sample(search_terms["단일_키워드"], min(15, len(search_terms["단일_키워드"]))):
        test_scenarios.append({
            "category": "단일 키워드",
            "query": keyword,
    ...
    # 5. 기술 용어 검색
    for term in random.sample(search_terms["기술_용어"], min(10, len(search_terms["기술_용어"]))):
        test_scenarios.append({
            "category": "기술 용어",
            "query": term,
            "expected_min_results": 1
    ...
    # 6. 한국어 용어 검색
    for term in random.sample(search_terms["한국어_용어"], min(10, len(search_terms["한국어_용어"]))):
        test_scenarios.append({
            "category": "한국어 용어",
            "query": term,
            "expected_min_results": 1
    ...
    # 7. 복합 쿼리 검색
    for query in random.sample(search_terms["복합_쿼리"], min(10, len(search_terms["복합_쿼리"]))):
        test_scenarios.append({
            "category": "복합 쿼리",
            "query": query,
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        if category not in results_by_category:
            results_by_category[category] = {"total": 0, "success": 0, "with_results": 0}
    ...
    for category, stats in sorted(results_by_category.items()):
        ...
        result_rate = stats["with_results"] / stats["total"] * 100 if stats["total"] > 0 else 0
        print(f"\n{category}:")

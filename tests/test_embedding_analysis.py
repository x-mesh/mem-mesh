...
async def analyze_embeddings():
    ...
    if understood_sim > thresh_sim:
        print(
            f"  ❗ 'threshold'가 'understood'와 더 유사함! ({understood_sim:.6f} > {thresh_sim:.6f})"
        )
        print("     이것이 검색 결과 이상 현상의 원인일 수 있습니다.")
    else:
        print(
            f"  ✅ 'threshold'가 'threshold1'과 더 유사함 ({thresh_sim:.6f} > {understood_sim:.6f})"
        )

    # 벡터 거리 분석
    print("\n📏 유클리드 거리 분석:")
    thresh_thresh1_dist = np.linalg.norm(
        embeddings["threshold"] - embeddings["threshold1"]
    )
    thresh_understood_dist = np.linalg.norm(
        embeddings["threshold"] - embeddings["understood"]
    )

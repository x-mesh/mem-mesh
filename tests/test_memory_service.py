...
from src.services.memory import MemoryService, MemoryNotFoundError, DatabaseError, EmbeddingError
...
class TestMemoryService:
    ...
    async def test_update_memory_metadata_only(self, memory_service):
        ...
        assert updated_memory.content_hash == original_memory.content_hash  # 해시도 변경되지 않음
    ...
    async def test_embedding_max_retries_exceeded(self, temp_db):
        ...
        with pytest.raises(EmbeddingError):
            await memory_service.create(
                content="Test content for max retries",
                source="test"
            )

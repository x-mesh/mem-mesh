...
async def fix_threshold_search():
    ...
    try:
        ...
        for memory in understood_memories:
            print(
                f"  - ID: {memory['id'][:8]}... | 프로젝트: {memory['project_id']} | 생성일: {memory['created_at']}"
            )
        ...
        if choice == "1":
            ...
            for i, memory in enumerate(understood_memories, 1):
                ...
                if new_content:
                    ...
                    print(
                        f"  ✅ 수정됨: {memory['id'][:8]}... -> '{new_content[:50]}...'"
                    )
        ...
        if choice in ["1", "2"]:
            ...
            for i, res in enumerate(result.results, 1):
                content = (
                    res.content[:50] + "..." if len(res.content) > 50 else res.content
                )

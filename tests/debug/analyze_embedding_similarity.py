...
def analyze_semantic_similarity(embedding_service):
    ...
    # 다양한 카테고리의 단어들
    word_categories = {
        "기술 용어": ["threshold", "threshold1", "parameter", "variable", "config", "setting"],
        "확인/이해": ["understood", "understand", "ok", "yes", "confirmed", "acknowledged"],
        "숫자 관련": ["threshold1", "value1", "count1", "index1", "number1"],
        "임계값 관련": ["threshold", "limit", "boundary", "cutoff", "maximum", "minimum"],
...
def analyze_vector_components(embedding_service):
    ...
    for idx in top_diff_indices:
        print(f"    차원 {idx}: threshold={thresh_emb[idx]:.4f}, understood={understood_emb[idx]:.4f}, 차이={diff_understood[idx]:.4f}")

...
class TestSearchService:
    ...
    async def test_search_with_results(self, search_service, memory_service):
        ...
        # Then
        assert len(response.results) >= 0  # sqlite-vec가 없어도 fallback으로 결과 반환 가능
    ...
    async def test_search_with_category_filter(self, search_service, memory_service):
        ...
        # Given
        await memory_service.create(
            content="Task memory for testing",
            category="task",
            source="test"
        )
        
        await memory_service.create(
            content="Bug memory for testing",
            category="bug",
            source="test"
        )
    ...
    async def test_search_with_recency_weight(self, search_service, memory_service):
        ...
        # Given
        await memory_service.create(
            content="Old memory for recency test",
            source="test"
        )
        ...
        await memory_service.create(
            content="New memory for recency test",
            source="test"
        )

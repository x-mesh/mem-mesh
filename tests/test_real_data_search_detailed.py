...
async def test_search_quality_metrics():
    ...
    real_world_queries = [
        ...
        # 8. 긴 쿼리
        {"query": "검색 품질을 개선하기 위한 UnifiedSearchService 구현", "description": "긴 쿼리 1"},
        ...
        try:
            ...
            if result_count > 0:
                ...
                metrics["detailed_results"].append({
                    ...
                    "quality": quality,
                    "top_result_preview": result.results[0].content[:100] if result.results else ""
                })
                
                print(f"{i:2d}. [{quality:4s}] {description:20s} | "
                      f"쿼리: '{query[:30]:30s}' | "
                      f"결과: {result_count:2d}개 | "
                      f"점수: {top_score:.3f} | "
                      f"시간: {search_time*1000:.1f}ms")
            else:
                print(f"{i:2d}. [없음] {description:20s} | "
                      f"쿼리: '{query[:30]:30s}' | "
                      f"결과: 0개")
            ...
            print(f"{i:2d}. [오류] {description:20s} | "
                  f"쿼리: '{query[:30]:30s}' | "
                  f"오류: {str(e)[:50]}")
    ...
    print(f"결과 있는 쿼리: {metrics['queries_with_results']}개 "
    ...
    print(f"  - 낮음 (<0.5): {metrics['low_quality_results']}개 "
    ...
    for i, result in enumerate(high_quality, 1):
        ...
        print(f"   점수: {result['top_score']:.3f} | 결과: {result['result_count']}개 | "
    ...
    quality_rate = metrics["high_quality_results"] / metrics["queries_with_results"] if metrics["queries_with_results"] > 0 else 0

...
def valid_project_id(draw):
    ...
    return draw(st.one_of(
        ...
        st.text(alphabet=st.characters(whitelist_categories=("Ll", "Nd"), whitelist_characters="-_"), 
                min_size=1, max_size=20).filter(lambda x: x and re.match(r'^[a-z0-9_-]+$', x))
    ))
...
def valid_category(draw):
    ...
    return draw(st.sampled_from([
        "task", "bug", "idea", "decision", "incident", "code_snippet"
    ]))
...
class TestConcurrencyProperties:
    ...
    @hyp_settings(max_examples=5, deadline=10000, suppress_health_check=[HealthCheck.function_scoped_fixture])
    @pytest.mark.asyncio
        ...
        **Validates: Requirements 4.2, 4.3**
        ...
        for writer_id, created_ids in writer_results:
            for memory_id in created_ids:
                memory = await memory_service.get(memory_id)
                assert memory is not None, f"Memory {memory_id} from writer {writer_id} was not saved"
...
class TestSettingsProperties:
    ...
    @given(
        ...
        api_base_url=st.text(min_size=10, max_size=50).map(lambda x: "http://" + x.replace(" ", "")),
    ...
    @given(
        invalid_mode=st.text().filter(lambda x: x not in ("direct", "api") and len(x) > 0)
    )
    @hyp_settings(max_examples=10, suppress_health_check=[HealthCheck.function_scoped_fixture])

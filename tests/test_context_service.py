...
from src.services.context import ContextService, ContextNotFoundError
from src.schemas.responses import ContextResponse, RelatedMemory
...
async def test_get_context_basic(context_service, memory_service):
    ...
    for i in range(len(context.timeline) - 1):
        current_time = datetime.fromisoformat(timeline_dict[context.timeline[i]].replace('Z', '+00:00'))
        next_time = datetime.fromisoformat(timeline_dict[context.timeline[i + 1]].replace('Z', '+00:00'))
...
async def test_get_context_with_project_filter(context_service, memory_service):
    ...
    all_project_ids.extend([mem.project_id for mem in context.related_memories if hasattr(mem, 'project_id')])

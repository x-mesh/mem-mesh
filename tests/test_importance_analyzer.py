...
class TestImportanceAnalyzer:
    ...
    def test_real_scenario_minor_fix(self, analyzer):
        """실제 시나리오: 사소한 수정"""
        result = analyzer.analyze("Typo in error message", tags=["cleanup"])
        assert result == 1

    def test_real_scenario_documentation(self, analyzer):
        """실제 시나리오: 문서 작업"""
        result = analyzer.analyze(
            "Add API documentation and usage examples", tags=["docs"]
        )
        assert result == 2
    ...
    def test_real_scenario_korean_mixed(self, analyzer):
        ...
        result = analyzer.analyze(
            "시스템 아키텍처 설계 및 구조 변경", tags=["architecture", "design"]
        )
        assert result == 5

    ...
    for query, description in test_queries:
        ...
        if result.results:
            ...
            for i, res in enumerate(result.results[:3], 1):
                print(f"  {i}. project_id={res.project_id}, score={res.similarity_score:.4f}")
                print(f"     content={res.content[:80]}...")
            
            # 프로젝트명 매칭 검증
            if query in ["mem-mesh", "mem"]:
                mem_mesh_results = [r for r in result.results if r.project_id and "mem" in r.project_id.lower()]
                if mem_mesh_results:
...
async def test_cache_ttl_configuration():
    ...
    # TTL 검증
    assert stats['caches']['embedding']['ttl'] == settings.cache_embedding_ttl, "Embedding TTL 불일치"
    assert stats['caches']['search']['ttl'] == settings.cache_search_ttl, "Search TTL 불일치"
    assert stats['caches']['context']['ttl'] == settings.cache_context_ttl, "Context TTL 불일치"

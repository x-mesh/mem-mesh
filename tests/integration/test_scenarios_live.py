...
class TestCrossProtocol:
    ...
    async def test_rest_create_mcp_search(
        ...
        r = await http.post("/api/memories", json={
    ...
    async def test_mcp_update_rest_verify(
        ...
        await mcp_tools_call(http, "update", {
            "memory_id": memory_id,
            "content": new_content,
            "category": "decision",
        }, session_id=mcp_session)
...
class TestRealWorldWorkflow:
    ...
    async def test_developer_session(
        ...
        # 1. Session resume
        resume = await mcp_tools_call(http, "session_resume", {
            "project_id": TEST_PROJECT_ID,
            "expand": "smart",
        }, session_id=mcp_session)
        ...
        await mcp_tools_call(http, "pin_complete", {
            "pin_id": pin_ids[0],
        }, session_id=mcp_session)

        promote = await mcp_tools_call(http, "pin_promote", {
            "pin_id": pin_ids[0],
        }, session_id=mcp_session)
        ...
        # 9. End session
        await mcp_tools_call(http, "session_end", {
            "project_id": TEST_PROJECT_ID,
...
class TestSearchAccuracy:
    ...
    async def test_category_filter(
        ...
        idea = await mcp_tools_call(http, "add", {
            "content": unique_content("Category filter test: an innovative idea about caching"),
            "project_id": TEST_PROJECT_ID,
            "category": "idea",
        }, session_id=mcp_session)
        ...
        decision = await mcp_tools_call(http, "add", {
            "content": unique_content("Category filter test: a decision about caching strategy"),
            "project_id": TEST_PROJECT_ID,
            "category": "decision",
        }, session_id=mcp_session)
        ...
        result = await mcp_tools_call(http, "search", {
            "query": "caching",
            "project_id": TEST_PROJECT_ID,
            "category": "idea",
            "limit": 10,
    ...
    async def test_recency_weight(
        ...
        for i in range(2):
            result = await mcp_tools_call(http, "add", {
                "content": unique_content(f"Recency test memory #{i}: server monitoring setup"),
                "project_id": TEST_PROJECT_ID,
                "category": "task",
            }, session_id=mcp_session)
            cleanup_memories.append(result["id"])
    ...
    async def test_batch_mixed_operations(
        ...
        result = await mcp_tools_call(http, "batch_operations", {
            "operations": [
                {
                    "type": "add",
                    "content": unique_content("Batch mixed: add operation in batch"),
                    "project_id": TEST_PROJECT_ID,
                    "category": "task",
                },
                {

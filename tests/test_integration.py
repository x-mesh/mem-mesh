...
async def test_add_search_workflow(services):
    ...
    await wait_until(check_search_results, timeout=5.0, error_msg="Search results did not appear")
...
async def test_add_context_workflow(services):
    ...
    await wait_until(check_context, timeout=5.0, error_msg="Context timeline incomplete")
...
async def test_add_update_search_workflow(services):
    ...
    await wait_until(check_updated_content, timeout=5.0, error_msg="Updated content not found in search")
...
async def test_add_delete_search_workflow(services):
    ...
    await wait_until(check_deletion, timeout=5.0, error_msg="Deleted memory still appearing or remaining memory missing")
...
async def test_cross_project_isolation(services):
    ...
    async def check_isolation():
        res_a = await search_service.search("authentication system", project_id="project-a", limit=10)
        res_b = await search_service.search("authentication system", project_id="project-b", limit=10)
        
        ids_a = [r.id for r in res_a.results]
        ids_b = [r.id for r in res_b.results]
        
        return (project_a_memory.id in ids_a and 
                project_a_memory.id not in ids_b and 
                project_b_memory.id in ids_b and 
                project_b_memory.id not in ids_a)
...
async def test_category_filtering(services):
    ...
    async def check_categories():
        res_task = await search_service.search("authentication", project_id="test-project", category="task", limit=10)
        res_bug = await search_service.search("authentication", project_id="test-project", category="bug", limit=10)
        
        ids_task = [r.id for r in res_task.results]
        ids_bug = [r.id for r in res_bug.results]
        
        return (task_memory.id in ids_task and 
                bug_memory.id not in ids_task and 
                bug_memory.id in ids_bug and 
                task_memory.id not in ids_bug)

    await wait_until(check_categories, timeout=5.0, error_msg="Category filtering failed")

...
class TestSearchServiceMetricsIntegration:
    ...
    async def test_search_collects_metrics(
        ...
        # When
        with patch.object(
            service.cache_manager, "get_cached_search", return_value=None
        ):
            with patch.object(
                service.cache_manager, "get_cached_embedding", return_value=None
            ):
                with patch.object(
                    service.cache_manager, "cache_embedding", return_value=None
                ):
                    with patch.object(
    ...
    async def test_empty_query_collects_metrics(
        ...
        # Given
        mock_db.get_recent_memories = AsyncMock(
            return_value=[
                {
                    "id": "1",
                    "content": "test",
...
class TestEmbeddingServiceMetricsIntegration:
    ...
    def test_embedding_without_metrics_collector(self):
        ...
        with patch("app.core.embeddings.service.SentenceTransformer") as mock_st:
            ...
            service = EmbeddingService(
                model_name="all-MiniLM-L6-v2", preload=True, metrics_collector=None
            )
...
class TestMetricsCollectorBuffering:
    ...
    async def test_buffer_auto_flush_on_size(self, mock_db):
        ...
        for i in range(3):
            await collector.collect_search_metric(
                query=f"query {i}", result_count=i, response_time_ms=100
            )
    ...
    async def test_manual_flush(self, mock_db):
        ...
        # Given
        collector = MetricsCollector(
            database=mock_db, buffer_size=100, flush_interval=3600
        )

        await collector.collect_search_metric(
            query="test", result_count=5, response_time_ms=100
        )
    ...
    async def test_buffer_stats(self, mock_db):
        ...
        # Given
        collector = MetricsCollector(
            database=mock_db, buffer_size=100, flush_interval=60
        )

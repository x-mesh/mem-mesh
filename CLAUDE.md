...
<!-- mem-mesh-hooks:BEGIN v21 -->
## mem-mesh Hook Rules (managed)

Generated by `mem-mesh hooks sync-project --target claude`. Prompt version: `21`. Project ID: `mem-mesh`.
...
3. **Save permanent memories selectively** — Use add() only for decision, bug, incident, idea, and code_snippet memories. Routine task state belongs in pins.
4. **Use context search** — When prior decisions, tasks, or design context are referenced, call search() before writing code.
5. **End sessions when requested** — When the user explicitly says the session is done, finish the request and then call session_end(project_id="mem-mesh").
<!-- mem-mesh-hooks:END v21 -->

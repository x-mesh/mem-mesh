...
class AddParams(BaseModel):
    """메모리 추가 요청 파라미터"""
    content: str = Field(min_length=10, max_length=20000)  # 10,000 -> 20,000으로 증가
    ...
    def validate_project_id(cls, v):
        if v is not None:
            ...
            if not re.match(r'^[a-z0-9_-]+$', v):
                raise ValueError("project_id must contain only lowercase letters, numbers, hyphens, and underscores")
        return v
...
class UpdateParams(BaseModel):
    """메모리 업데이트 요청 파라미터"""
    content: Optional[str] = Field(default=None, min_length=10, max_length=20000)  # 10,000 -> 20,000으로 증가
    category: Optional[str] = Field(default=None)
    tags: Optional[List[str]] = Field(default=None)
...
class StatsParams(BaseModel):
    ...
    start_date: Optional[str] = Field(default=None, description="시작 날짜 (YYYY-MM-DD)")

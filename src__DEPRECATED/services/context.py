...
class ContextService:
    ...
    async def _find_related_memories(
        ...
        try:
            ...
            related_memories = await self._text_search(primary_memory, depth, project_id)
    ...
    async def _vector_search(
        ...
        try:
            ...
            cursor.execute(f"""
                ...
                LIMIT ?
            """, params + [query_bytes, query_bytes, 1.0 - self.similarity_threshold, depth * 5])
            
            rows = cursor.fetchall()
            
            # RelatedMemory 객체로 변환 및 관계 분류
            related_memories = []
            primary_created_at = datetime.fromisoformat(primary_memory.created_at.replace('Z', '+00:00'))
            ...
            for row in rows:
                ...
                # 관계 분류
                relationship = self._classify_relationship(primary_created_at, created_at, similarity_score)
    ...
    async def _text_search(
        ...
        try:
            ...
            for row in rows:
                ...
                # 간단한 텍스트 유사도 계산
                similarity_score = self._calculate_text_similarity(primary_memory.content, row[1])
    ...
    async def _expand_search(
        ...
        for memory in initial_memories[:3]:  # 상위 3개만 확장
            try:
                # 텍스트 기반 검색 사용 (sqlite-vec 지원 없음)
                additional_memories = await self._text_search_for_expansion(memory, project_id, processed_ids)
                additional_memories = await self._text_search_for_expansion(memory, project_id, processed_ids)
    ...
    async def _create_timeline(
        ...
        # 주요 메모리 추가
        primary_time = datetime.fromisoformat(primary_memory.created_at.replace('Z', '+00:00'))
    ...
    async def _vector_search_for_expansion(
        ...
        try:
            ...
            for row in rows:
                ...
                relationship = self._classify_relationship(memory_time, created_at, similarity_score)
    ...
    async def _text_search_for_expansion(
        ...
        try:
            ...
            for row in rows:
                ...
                similarity_score = self._calculate_text_similarity(memory.content, row[1]) * 0.8

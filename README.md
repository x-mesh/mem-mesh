...
| `pin_complete` | 핀 완료 | pin_id |
| `pin_promote` | 핀을 영구 메모리로 승격 | pin_id |
| `session_resume` | 프로젝트 세션 재개 | project_id, expand, limit |
| `session_end` | 세션 종료 | project_id, summary |
| `batch_operations` | 다중 연산 (토큰 절감) | operations (add/search 등 배열) |
...
- `session_resume`: 이전 세션의 미완료 핀과 맥락을 복원. `expand="smart"`는 중요도×상태 기반 선택적 로드로 ~60% 토큰 절감.
...
```
pin_add(content, project_id)  →  작업 수행  →  pin_complete(pin_id)  →  pin_promote(pin_id)
                                                                          (중요도 ≥ 4일 때)
```
...
| 유형 | Pin | 예시 |
|---|---|---|
| 코드 변경 / 구현 / 리팩토링 | ✅ | "버그 수정", "인증 모듈 구현" |
| 설정 / 인프라 작업 | ✅ | "Docker 설정 변경", "hook 설치" |
| 다단계 작업 | ✅ 각 단계별 | "API 설계 + 구현" → 2개 pin |
| 질문 / 설명 / 조회 | ❌ | "이 코드 뭐야?", "상태 확인해" |
...
**승격(promote):** 완료된 핀 중 장기 참조 가치가 있는 것(아키텍처 결정, 핵심 버그 해결 등)만 `pin_promote`로 영구 메모리에 저장합니다.
...
3. 작업 추적  → pin_add → pin_complete → (중요 시 pin_promote)
4. 영구 저장  → decision / bug / incident / idea / code_snippet 만
5. 세션 종료  → session_end(project_id, summary)

...
더 안정적인 연결을 위해 순수 MCP 프로토콜 구현을 사용할 수 있습니다:
...
**✅ 권장사항**: `app.pure_mcp`는 upnote MCP 서버를 참조하여 구현된 순수 MCP 프로토콜로, qwen, gemini 등 다양한 AI 클라이언트에서 안정적으로 작동합니다.
...
**⚠️ SSL 검증 우회 주의사항**:
- `MEM_MESH_IGNORE_SSL=1`은 SSL 인증서 검증을 완전히 비활성화합니다
- 신뢰할 수 있는 네트워크 환경에서만 사용하세요
- 회사 방화벽이나 프록시 환경에서 SSL 인증서 문제가 발생할 때만 사용하세요

...
```
session_resume(expand="smart") → pin_add → [work] → pin_complete(promote=true) → session_end
```

- **시작**: `session_resume(project_id, expand="smart", limit=10)` — 중요도×상태 기반 선택적 로드 (~60% 토큰 절약)
- **Stale 자동 정리**: session_resume 시 오래된 핀 자동 완료 (in_progress 7일, open 30일)
- **작업 추적**: `pin_add(content, project_id, importance=3)` → `pin_complete(pin_id, promote=true)`
  - 핀 기본 상태: `in_progress`. 나중 작업은 `open`으로 미리 등록 가능
  - importance 생략 시 내용 기반 자동 추정
  - 응답(compact): `{id, importance, status}`
- **importance ≥ 4**: `suggest_promotion` 제안 (promote 미사용 시 별도 `pin_promote(pin_id)`)

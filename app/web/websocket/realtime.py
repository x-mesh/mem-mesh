...
class RealtimeNotifier:
    ...
    async def notify_memory_created(memory_data: Dict[str, Any]) -> None:
        ...
        if project_id:
            project_sent = await connection_manager.broadcast_to_project(project_id, message)
            logger.debug(f"Memory created notification sent to {total_sent} clients ({project_sent} project subscribers)")
        else:
            logger.debug(f"Memory created notification sent to {total_sent} clients")
    
    @staticmethod
    async def notify_memory_updated(memory_id: str, memory_data: Dict[str, Any]) -> None:
        """메모리 수정 알림"""
        message = {
        ...
        if project_id:
            project_sent = await connection_manager.broadcast_to_project(project_id, message)
            logger.debug(f"Memory updated notification sent to {total_sent} clients ({project_sent} project subscribers)")
        else:
            logger.debug(f"Memory updated notification sent to {total_sent} clients")
    
    @staticmethod
    async def notify_memory_deleted(memory_id: str, project_id: Optional[str] = None) -> None:
        """메모리 삭제 알림"""
        message = {
        ...
        if project_id:
            project_sent = await connection_manager.broadcast_to_project(project_id, message)
            logger.debug(f"Memory deleted notification sent to {total_sent} clients ({project_sent} project subscribers)")
        else:

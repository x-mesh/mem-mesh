...
class MemoriesPage extends HTMLElement {
    ...
    return `
      <div class="recent-item mem-row${isSelected ? ' mem-selected' : ''}" data-memory-id="${esc(mem.id)}" role="button" tabindex="0">
        ...
        <span class="mem-row-actions">
          <button class="mem-action-btn mem-relay-btn${isRelayShareableKind(mem) ? '' : ' mem-relay-btn--off'}" data-id="${esc(mem.id)}" title="${isRelayShareableKind(mem) ? 'Share to relay' : `'${esc(mem.category)}' is not team-shareable`}">
  ...
  async shareMemoryToRelay(memoryId) {
    ...
    const mem = this.memories.find(m => m.id === memoryId);
    if (!isRelayShareableKind(mem)) {
      this.showToast(`'${mem?.category}' memories aren't team-shareable`, 'info');
      return;
    }
    ...
    panel.innerHTML = `
      <div class="mem-peek-header">
        ...
        <div class="mem-peek-actions">
          <button class="mem-action-btn mem-peek-relay${isRelayShareableKind(m) ? '' : ' mem-relay-btn--off'}" data-id="${esc(m.id)}" title="${isRelayShareableKind(m) ? 'Share to relay' : `'${esc(m.category)}' is not team-shareable`}">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
          </button>
...
style.textContent = `
  ...
  .mem-relay-btn--off { opacity: 0.35; }
  .mem-relay-btn--off:hover { color: inherit; }

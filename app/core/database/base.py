    ...
    async def connect(self) -> None:
        ...
        async with self._lock:
            ...
            try:
                ...
                if not vec_loaded:
                    logger.warning(
                        "sqlite-vec not available, using fallback text search only"
                    )
        ...
        try:
            ...
            if SQLITE_VEC_AVAILABLE:
                try:
                    ...
                    test_result = self.connection.execute(
                        "SELECT vec_version()"
                    ).fetchone()
                    if test_result:
    ...
    async def check_embedding_model_consistency(
        ...
        if stored_model is None:
            ...
            if count > 0:
                ...
                result["message"] = (
                    f"⚠️ 기존 메모리 {count}개가 있지만 임베딩 모델 정보가 없습니다. 현재 모델({current_model})로 메타데이터를 설정합니다."
                )
                ...
                result["message"] = (
                    f"✅ 새 데이터베이스입니다. 임베딩 모델: {current_model} (dim: {current_dim})"
                )
                await self.set_embedding_metadata("embedding_model", current_model)
                await self.set_embedding_metadata(
                    "embedding_dimension", str(current_dim)
        ...
        if stored_model != current_model:
            ...
            result["message"] = (
                f"✅ 임베딩 모델 일치: {current_model} (dim: {current_dim})"
            )

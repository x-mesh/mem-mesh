...
class AlertService:
    ...
    async def start_background_check(self, interval_minutes: int = 5):
        ...
        if self._check_task is None or self._check_task.done():
            ...
            logger.info(f"Alert background check started (interval: {interval_minutes}min)")
    ...
    async def check_thresholds(self) -> List[Alert]:
        ...
        try:
            ...
            if metrics["total_searches"] < min_samples:
                logger.debug(f"Not enough samples ({metrics['total_searches']} < {min_samples})")
                return []
            ...
            if metrics["no_results_rate"] > self.thresholds["max_no_results_rate"]:
                severity = AlertSeverity.ERROR if metrics["no_results_rate"] > 30 else AlertSeverity.WARNING
                alert = await self._create_alert_if_not_exists(
            ...
            if metrics["avg_response_time_ms"] > self.thresholds["max_response_time_ms"]:
                severity = AlertSeverity.ERROR if metrics["avg_response_time_ms"] > 2000 else AlertSeverity.WARNING
                alert = await self._create_alert_if_not_exists(

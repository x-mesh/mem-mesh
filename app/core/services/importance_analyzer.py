...
class ImportanceAnalyzer:
    ...
    def _load_importance_keywords(self) -> Dict[int, List[str]]:
        ...
        return {
            5: [
                # 영어 키워드
                "architecture", "design", "critical", "breaking", "major",
                "system", "infrastructure", "security", "migration",
                # 한국어 키워드
                "아키텍처", "설계", "중대", "중요", "시스템",
                "인프라", "보안", "마이그레이션", "구조"
            ],
            4: [
                # 영어 키워드
                "feature", "implement", "refactor", "optimize", "enhancement",
                "performance", "integration", "api", "service",
                # 한국어 키워드
                "기능", "구현", "리팩토링", "리팩터링", "최적화",
                "성능", "통합", "서비스"
            ],
            ...
            2: [
                # 영어 키워드
                "test", "doc", "comment", "documentation", "readme",
    ...
    def analyze(
        ...
        if max_score == 0:
            # 매칭된 키워드가 없으면 기본값 반환
            logger.debug(f"No keywords matched, returning default importance: {self.DEFAULT_IMPORTANCE}")
            return self.DEFAULT_IMPORTANCE

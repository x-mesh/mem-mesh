...
class SearchWarmupService:
    ...
    async def warmup(self, embedding_service, db, cache_manager) -> dict:
        """
        검색 시스템 워밍업 수행
    ...
    def get_warmup_status(self) -> dict:
        ...
        if not self.is_warmed_up:
            return {"is_warmed_up": False, "message": "Not warmed up yet"}

        warmup_time = (
            (self.warmup_end_time - self.warmup_start_time).total_seconds()
        ...
        return {
            ...
            "warmup_start": (
                self.warmup_start_time.isoformat() if self.warmup_start_time else None
            ),
            "warmup_end": (
                self.warmup_end_time.isoformat() if self.warmup_end_time else None
            ),
        }

    ...
    async def _run_migration(
        ...
        while True:
            ...
            for memory in memories:
                ...
                self._migration_progress["percent"] = int((processed / stats["total"]) * 100)
        ...
        # 메타데이터 업데이트
        now = datetime.now(timezone.utc).isoformat()
        await self.db.set_embedding_metadata("embedding_model", self.embedding_service.model_name)
        ...
        self._migration_progress["message"] = f"Migration completed: {stats['migrated']} migrated, {stats['failed']} failed"

...
class ContextOptimizer:
    ...
    def __init__(self, session_service: "SessionService"):
        ...
        # 기본 파라미터 (의도 불명확 시)
        self._default_params = ContextLoadingParams(expand=False, limit=10, min_importance=1)
    ...
    async def adjust_for_intent(
        ...
        if intent.urgency > 0.8:
            ...
            logger.debug(f"High urgency detected ({intent.urgency:.2f}): limit={limit}, min_importance={min_importance}")
        ...
        if intent.specificity > 0.8:
            ...
            limit = min(limit, 3)
            expand = True
            logger.debug(f"High specificity detected ({intent.specificity:.2f}): limit={limit}, expand=True")
        elif intent.specificity < 0.3:
            # 모호한 경우: 넓은 범위 탐색
            limit = max(limit, 15)
            expand = False
            min_importance = 1
            logger.debug(f"Low specificity detected ({intent.specificity:.2f}): limit={limit}, expand=False")
    ...
    async def load_context_for_search(
        ...
        if expand and context.pins and min_importance > 1:
            ...
            context.pins = [
                pin for pin in context.pins 
                if pin.importance >= min_importance
            ]

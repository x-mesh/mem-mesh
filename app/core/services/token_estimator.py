...
class TokenEstimator:
    ...
    def __init__(self, default_model: str = "gpt-4"):
        ...
        try:
            ...
            logger.info(
                f"TokenEstimator initialized with default model: {default_model}"
            )
    ...
    def estimate_tokens(self, content: str, model: Optional[str] = None) -> int:
        """
        텍스트의 예상 토큰 수를 계산합니다.
        ...
        try:
            ...
            logger.debug(
                f"Estimated {token_count} tokens for {len(content)} characters (model: {model})"
            )
            return token_count
    ...
    def estimate_tokens_batch(
        self, contents: list[str], model: Optional[str] = None
    ) -> list[int]:
        """
    ...
    def estimate_tokens_dict(self, data: dict, model: Optional[str] = None) -> int:
        """

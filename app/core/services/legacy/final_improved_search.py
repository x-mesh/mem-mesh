...
class FinalImprovedSearch:
    ...
    async def search(
        ...
        final_results = self._merge_and_score(
            text_results, vector_results, query, expanded_terms, project_filter
        )
    ...
    def _text_search(
        self, terms: List[str], project_filter: Optional[str]
    ) -> Dict[str, dict]:
        """텍스트 기반 검색"""
        results = {}
    ...
    def _vector_search(
        self, query: str, project_filter: Optional[str], limit: int
    ) -> Dict[str, dict]:
        """벡터 기반 검색"""
        results = {}
        conn = self.db.connection

...
class PinStatistics(BaseModel):
    ...
    avg_lead_time_hours: Optional[float] = Field(
        None, description="평균 리드 타임 (시간)"
    )
    promotion_candidates: int = Field(
        ..., description="승격 후보 핀 수 (importance >= 4)"
    )
...
class OptimizedSessionContext(BaseModel):
    ...
    recommendations: List[str] = Field(
        default_factory=list, description="권장사항 목록"
    )
...
class SessionStatRecord(BaseModel):
    ...
    event_type: str = Field(
        ..., description="이벤트 타입 (resume, search, pin_add, end)"
    )
...
class TokenUsageRecord(BaseModel):
    ...
    operation_type: str = Field(
        ..., description="작업 타입 (session_resume, search, context_load)"
    )
...
class TokenSavingsReport(BaseModel):
    ...
    optimization_details: Dict[str, Any] = Field(
        default_factory=dict, description="최적화 상세 정보"
    )

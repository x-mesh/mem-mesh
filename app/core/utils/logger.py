...
def get_formatter() -> logging.Formatter:
    ...
    log_format = (
        os.getenv("MCP_LOG_FORMAT") or 
        os.getenv("MEM_MESH_LOG_FORMAT") or 
        "text"
    ).lower()
...
class MemMeshLogger:
    ...
    def _get_log_level(self) -> str:
        ...
        return (
            os.getenv("MCP_LOG_LEVEL") or 
            os.getenv("MEM_MESH_LOG_LEVEL") or 
            "INFO"
        )
    ...
    def _get_log_file(self) -> str:
        ...
        return (
            os.getenv("MCP_LOG_FILE") or 
            os.getenv("MEM_MESH_LOG_FILE") or 
            ""
        )
    ...
    def _get_log_output(self) -> str:
        ...
        return (
            os.getenv("MCP_LOG_OUTPUT") or 
            os.getenv("MEM_MESH_LOG_OUTPUT") or 
            "console"
        )
    ...
    def info_with_details(self, base_msg: str, details: Dict[str, Any] = None, **kwargs) -> None:
        """
...
def setup_logging(logger_name: Optional[str] = None) -> Union[MemMeshLogger, logging.Logger]:
    """
    ...
    if log_file:
        try:
            ...
            print(f"Warning: Could not create log directory for {log_file}: {e}", file=sys.stderr)
...
def setup_simple_logger(name: str) -> logging.Logger:
    ...
    simple_logger.info(f"Logging initialized: level={log_level_str}, output={log_output}, file={log_file or 'none'}")

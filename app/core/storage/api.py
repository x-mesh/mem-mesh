...
class APIStorageBackend(StorageBackend):
    ...
    async def add_memory(self, params: AddParams) -> AddResponse:
        ...
        try:
            logger.debug(f"Adding memory via API with content length: {len(params.content)}")
    ...
    async def search_memories(self, params: SearchParams) -> SearchResponse:
        ...
        try:
            ...
            logger.info(f"Search completed via API, found {len(result.results)} results")
            return result
    ...
    async def get_context(
        ...
        try:
            logger.debug(f"Getting context via API for memory_id: {memory_id}, depth: {depth}")
            ...
            logger.info(f"Context retrieved via API for memory {memory_id}, found {len(result.related_memories)} related memories")
            return result
    ...
    async def get_stats(self, params: StatsParams) -> StatsResponse:
        ...
        try:
            ...
            logger.info(f"Stats retrieved successfully via API, total memories: {result.total_memories}")
            return result

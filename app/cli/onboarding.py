...
def _setup_server_docker(url: str) -> tuple[bool, str]:
    ...
    if not compose_file:
        # Extract port from URL for port mapping
        port = 8000
        if ":" in url.split("//")[-1]:
            try:
        ...
        print(f"  {dim('No docker-compose.yml found. Generating one...')}")
        compose_file = _generate_compose_file(port=port)

...
def render_claude_project_rules(project_id: str = "mem-mesh") -> str:
    """Render a managed CLAUDE.md block for project-local Claude Code rules."""
    ...
    return (
        f"<!-- mem-mesh-hooks:BEGIN v{PROMPT_VERSION} -->\n"
        "## mem-mesh Hook Rules (managed)\n\n"
        "Generated by `mem-mesh hooks sync-project --target claude`. "
        f"Prompt version: `{PROMPT_VERSION}`. Project ID: `{project_id}`.\n\n"
        "### Mandatory Gates\n\n"
        f"1. Session Gate: call `{resume_call}` at session start and report "
        "`pins_count`, `in_progress_pins`, `open_pins`, `completed_pins`.\n"
        "2. Pin Gate: before task work, decide whether this request needs a pin. "
        f"Create when: {PIN_CRITERIA.create_when}. Skip when: {PIN_CRITERIA.skip_when}.\n"
        "3. Pin Response: state exactly one of `Pin created: <id>` or "
        "`No pin created: <reason>`.\n"
        "4. Completion Gate: if a pin was created, call `pin_complete` before the "
        "final response. Do not reuse unrelated `in_progress` pins.\n\n"
        "### Pin Call Format\n\n"
        "```text\n"
        f"{pin_format}\n"
        "```\n\n"
        "### Active Hook Rules\n\n"
        f"{render_rules_text(project_id)}\n"
        f"<!-- mem-mesh-hooks:END v{PROMPT_VERSION} -->"
    )
